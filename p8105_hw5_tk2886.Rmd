...
baltimore_homicide <-
  ...
  summarize(total_homicide_bm = n(),
            unsolved_homicide_bm = sum(disposition == "Closed without arrest" | 
                                         disposition == "Open/No arrest"))
...
baltimore_statistics <-
  prop.test(x = baltimore_homicide %>% pull(unsolved_homicide_bm), 
            n = baltimore_homicide %>% pull(total_homicide_bm))
```
...
```{r}
summarizing_homicide = 
  function(total, unsolved){
    summarizing_city = prop.test(total, unsolved) %>%
      broom::tidy() %>%
    select(estimate, conf.low, conf.high)
...
```{r}
city_homicide_statistics = 
  map2(.x = homicides_summary %>% pull(unsolved_homicide), 
       .y = homicides_summary %>% pull(total_homicide), 
       ~summarizing_homicide(.x, .y)) %>%
  bind_rows() %>%
  mutate(city_state = homicides_summary %>% pull(city)) %>%
